...
# 纯 Linux Bash 脚本实现，不依赖 Python
#
# 作者：AI Security Auditor
# 版本：v2.5.0 (修复 grep -oP 兼容性问题，使用 Perl 替代)
#
...
if [[ -z "${HOME:-}" ]]; then
    ...
    export HOME
    echo "警告：HOME 变量未设置，已自动设置为: ${HOME}"
fi
...
# 全局变量：Agent ID 映射（用于跨模块引用）
declare -A AGENT_ID_MAP
declare -A AGENT_PATH_MAP
AGENT_COUNTER=0
...
# ============================================================================
# 兼容性函数 (替代 grep -oP，支持更多 Linux 发行版)
# 说明：某些 Linux 发行版（如 Alpine）的 grep 不支持 -P，使用 perl 替代
# ============================================================================
...
collect_host_info() {
    ...
    # 获取主机 IP 地址（取第一个非 127.0.0.1 的 IP）
    ...
    log "INFO" "主机名：${HOST_NAME}, IP: ${HOST_IP}"
}
...
discover_ai_agents() {
    ...
    for cmd in openclaw opencode claude claude-code cline aider ollama llama zed cursor nanobot; do
        if command -v "${cmd}" &>/dev/null; then
            ...
            AGENT_ID_MAP["${cmd}"]="${agent_id}"
            AGENT_PATH_MAP["${agent_id}"]="${path}"
            ((count++))
        ...
        if [[ -d "${base}" ]]; then
            for pattern in ".openclaw" ".opencode" ".claude" ".nanobot"; do
                while IFS= read -r dir; do
                    ...
                    AGENT_ID_MAP["${agent_name}:${dir}"]="${agent_id}"
                    AGENT_PATH_MAP["${agent_id}"]="${dir}"
                    ((count++))
    ...
    if command -v npm &>/dev/null; then
        for pkg in claude claude-code openclaw opencode nanobot; do
            if npm list -g --depth=0 2>/dev/null | grep -q "${pkg}"; then
                ...
                AGENT_ID_MAP["npm:${pkg}"]="${agent_id}"
                AGENT_PATH_MAP["${agent_id}"]="${npm_root}"
                ((count++))
    ...
    if command -v pip3 &>/dev/null; then
        for pkg in ollama llama aider; do
            if pip3 list 2>/dev/null | grep -qi "${pkg}"; then
                ...
                AGENT_ID_MAP["pip:${pkg}"]="${agent_id}"
                AGENT_PATH_MAP["${agent_id}"]="${loc}"
                ((count++))
...
extract_mcp_config() {
    ...
    while IFS= read -r mcp_file; do
        ...
        # 检查文件大小（跳过大于 1MB 的文件）
        local file_size=$(stat -c%s "${mcp_file}" 2>/dev/null || echo "0")
...
extract_skills() {
    ...
    while IFS= read -r skill_md_path; do
        ...
        # 从 AGENT_ID_MAP 获取 agent_id
        for key in "${!AGENT_ID_MAP[@]}"; do
            if [[ "${key}" == *"${agent_name}"* ]]; then
                agent_id="${AGENT_ID_MAP[${key}]}"
                break
            fi
        done
...
extract_api_configs() {
    ...
    # 遍历已发现的 Agent
    for agent_key in "${!AGENT_ID_MAP[@]}"; do
        local agent_id="${AGENT_ID_MAP[${agent_key}]}"
        local agent_name="${agent_key%%:*}"
        local config_base="${agent_config_paths[${agent_name}]:-}"
        ...
        for agent_dir in "${HOME}/.openclaw/agents"/*; do
            ...
            if [[ -f "${models_json}" ]]; then
                ...
                for provider in ${provider_names}; do
                    ...
                    if [[ -n "${api_key}" ]]; then
                        local masked_key=$(mask_api_key "${api_key}")
                        local agent_id="${AGENT_ID_MAP[openclaw]:-A000}"
...
scan_scripts() {
    ...
    for base in "${HOME}/.claude" "${HOME}/.openclaw" "${HOME}/.opencode" /opt; do
        if [[ -d "${base}" ]]; then
            local agent_name="${agent_dir_map[${base}]:-unknown}"
            local agent_id="${AGENT_ID_MAP[${agent_name}]:-A000}"
...
generate_sbom() {
    ...
    local all_agent_ids=""
    for key in "${!AGENT_ID_MAP[@]}"; do
        all_agent_ids+="\"${AGENT_ID_MAP[${key}]}\","
    done
